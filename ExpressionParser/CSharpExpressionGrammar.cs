...
namespace My.ExpressionParser
{
    ...
    [Language("c# expression", "3.5", "Sample c# expression grammar")]
    public class CSharpExpressionGrammar : Grammar
    {
        TerminalSet _skipTokensInPreview = new TerminalSet(); //used in token preview for conflict resolution
        public CSharpExpressionGrammar()
        {
            ...
            var qual_name_with_targs_typeof = new NonTerminal("qual_name_with_targs_typeof");
            var qual_name_segment_typeof = new NonTerminal("qual_name_segment_typeof");
            var qual_name_segments_opt_typeof = new NonTerminal("qual_name_segments_opt_typeof");
            var type_or_void_typeof = new NonTerminal("type_or_void_typeof", "type or void");
            var type_ref_typeof = new NonTerminal("type_ref_typeof");
            var typeof_ref_expression = new NonTerminal("typeof_ref_expression");
            var typeof_parenthesized_expression = new NonTerminal("typeof_parenthesized_expression");
            ...
            var member_invoke = new NonTerminal("member_invoke");
            var member_indexer = new NonTerminal("member_indexer");
            ...
            var array_creation_args = new NonTerminal("array_creation_args");
            var object_creation_args = new NonTerminal("object_creation_args");
            ...
            var anonymous_type_creation_expression = new NonTerminal("anonymous_type_creation_expression");
            ...
            var anonymous_array_creation_expression = new NonTerminal("anonymous_array_creation_expression");
            var anonymous_object_initializer = new NonTerminal("anonymous_object_initializer");
            var anonymous_array_initializer = new NonTerminal("anonymous_array_initializer");
            var member_declarator = new NonTerminal("member_declarator");
            var member_declarator_list = new NonTerminal("member_declarator_list");
            var element_declarator = new NonTerminal("element_declarator");
            var element_declarator_list = new NonTerminal("element_declarator_list");
            ...
            var type_ref_expression = new NonTerminal("type_ref_expression");
            var typecast_parenthesized_expression = new NonTerminal("typecast_parenthesized_expression");
            ...
            //LINQ Support
            var query_from_expression = new NonTerminal("query_from_expression");
            var query_let_expression = new NonTerminal("query_let_expression");
            var query_where_expression = new NonTerminal("query_where_expression");
            var query_select_expression = new NonTerminal("query_select_expression");
            var query_joinon_expression = new NonTerminal("query_joinon_expression");
            var query_into_expression = new NonTerminal("query_into_expression");
            var query_join_expression = new NonTerminal("query_join_expression");
            var query_groupinto_expression = new NonTerminal("query_groupinto_expression");
            var query_orderby_order = new NonTerminal("query_orderby_order");
            var query_orderby_item = new NonTerminal("query_orderby_item");
            var query_orderby_list = new NonTerminal("query_orderby_list");
            var query_orderby_expression = new NonTerminal("query_orderby_expression");
            var query_end_expression = new NonTerminal("query_end_expression");
            var query_group_expression = new NonTerminal("query_group_expression");
            var query_list = new NonTerminal("query_list");
            var query_expression_opt = new NonTerminal("query_expression_opt");
            var query_groupjoin_expression = new NonTerminal("query_groupjoin_expression");
            #endregion
            ...
            this.MarkTransient(namespace_member_declaration, member_declaration, type_declaration, statement, embedded_statement, expression, type_ref_expression);
 ...
 */
            ...
            qual_name_segment.Rule = dot + identifier
                                    | "::" + identifier
                                    | type_argument_list;
            qual_name_with_targs.Rule = identifier_or_builtin + qual_name_segments_opt + qmark_opt;

            qual_name_segments_opt_typeof.Rule = MakeStarRule(qual_name_segments_opt_typeof, null, qual_name_segment_typeof);
            qual_name_segment_typeof.Rule = dot + identifier
                                    | "::" + identifier
                                    | type_argument_list
                                    | type_argument_list_opt;
            qual_name_with_targs_typeof.Rule = identifier_or_builtin + qual_name_segments_opt_typeof + qmark_opt;
            type_or_void_typeof.Rule = qual_name_with_targs_typeof | "void";
            type_ref_typeof.Rule = type_or_void_typeof + rank_specifiers_opt + generic_dimension_specifier_opt;
            generic_dimension_specifier_opt.Rule = Empty | lt + commas_opt + ">";

            var comma_list_opt = new NonTerminal("comma_list_opt");
            var comma_item = new NonTerminal("comma_item");
            comma_item.Rule = comma;
            comma_list_opt.Rule = MakeStarRule(comma_list_opt, Empty, comma_item);

            type_argument_list.Rule = lt + type_ref_list + ">";
            type_argument_list_opt.Rule = lt + comma_list_opt + ">";
            ...
            type_ref.Rule = type_or_void + rank_specifiers_opt + generic_dimension_specifier_opt;
            type_ref_list.Rule = MakePlusRule(type_ref_list, comma, type_ref);
                        
            rank_specifier.Rule = Obr + comma_list_opt + Cbr;
            ...
            var full_expression = new NonTerminal("full_expression");
            full_expression.Rule = assignment_expression
                                   | expression;
            assignment_expression.Rule = member_access + assignment_operator + full_expression;
            MarkTransient(full_expression);
            ...
            conditional_expression.Rule = expression + qmark + expression + colon + expression;// + ReduceThis();
            bin_op_expression.Rule = expression + bin_op + expression;

            typecast_expression.Rule = typecast_parenthesized_expression + primary_expression;
            ...
            dim_specifier.Rule = Obr + expression_list + Cbr;
            dim_specifier_opt.Rule = dim_specifier.Q();
            literal.Rule = Number | StringLiteral | CharLiteral | "true" | "false" | "null";
            type_ref_expression.Rule = type_ref | typecast_parenthesized_expression;
            typecast_parenthesized_expression.Rule = Lpar + type_ref_expression + Rpar;
            parenthesized_expression.Rule = Lpar + full_expression + Rpar;
            ...
            member_access_segment.Rule = dot + identifier
                           ...
                           | member_invoke
                           | member_indexer;
            array_indexer.Rule = Obr + expression_list + Cbr;

            member_invoke.Rule = dot + identifier + argument_list_par
                                | dot + identifier + type_argument_list + argument_list_par
                                | argument_list_par
                                | type_argument_list + argument_list_par;
            member_indexer.Rule = dot + identifier + array_indexer
                                  | array_indexer;
            ...
            object_creation_expression.Rule = "new" + qual_name_with_targs + creation_args + list_initializer_opt;
            creation_args.Rule = array_creation_args | object_creation_args;
            array_creation_args.Rule = dim_specifier | rank_specifier;
            object_creation_args.Rule = Empty | argument_list_par;

            anonymous_type_creation_expression.Rule = anonymous_object_creation_expression | anonymous_array_creation_expression;
            anonymous_object_creation_expression.Rule = "new" + anonymous_object_initializer;
            anonymous_array_creation_expression.Rule = "new" + Obr + Cbr + anonymous_array_initializer;
            anonymous_object_initializer.Rule = Lbr + Rbr | Lbr + member_declarator_list + comma_opt + Rbr;
            anonymous_array_initializer.Rule = Lbr + Rbr | Lbr + element_declarator_list + comma_opt + Rbr;
            member_declarator.Rule = identifier + "=" + expression;
            member_declarator_list.Rule = MakePlusRule(member_declarator_list, comma, member_declarator);
            element_declarator.Rule = expression;
            element_declarator_list.Rule = MakePlusRule(element_declarator_list, comma, element_declarator);
            //typeof
            typeof_ref_expression.Rule = type_ref_typeof | typeof_parenthesized_expression;
            typeof_parenthesized_expression.Rule = Lpar + typeof_ref_expression + Rpar;
            typeof_expression.Rule = "typeof" + typeof_parenthesized_expression;
            //checked, unchecked
            ...
            //Queries
            query_from_expression.Rule = "from" + anonymous_function_parameter_decl + "in" + expression;
            query_let_expression.Rule = "let" + identifier + "=" + expression;
            query_where_expression.Rule = "where" + expression;
            query_select_expression.Rule = "select" + expression;
            query_joinon_expression.Rule = "join" + anonymous_function_parameter_decl +
                                            "in" + expression + 
                                            "on" + expression + "equals" + expression;
            query_into_expression.Rule = "into" + identifier | Empty;
            query_join_expression.Rule = query_joinon_expression;
            query_groupjoin_expression.Rule = query_joinon_expression + query_into_expression;
            query_group_expression.Rule = "group" + identifier + "by" + expression;
            query_groupinto_expression.Rule = query_group_expression + query_into_expression;
            query_orderby_order.Rule = Empty | "ascending" | "descending";
            query_orderby_item.Rule = expression | expression + query_orderby_order;
            query_orderby_list.Rule = MakePlusRule(query_orderby_list, comma, query_orderby_item);
            query_orderby_expression.Rule = "orderby" + query_orderby_list;
            query_expression_opt.Rule = Empty | query_from_expression
                                        | query_let_expression
                                        | query_join_expression
                                        | query_groupjoin_expression
                                        | query_where_expression
                                        | query_groupinto_expression
                                        | query_orderby_expression;
            query_list.Rule = MakePlusRule(query_list, null, query_expression_opt);
            query_end_expression.Rule = query_select_expression | query_group_expression;
            query_expression.Rule = query_from_expression + query_list + query_end_expression 
                                    | query_from_expression + query_end_expression;
