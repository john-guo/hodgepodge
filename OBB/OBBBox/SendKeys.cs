...
namespace OBB
{
    ...
    public static class MySendKeys
    {
        ...
        {
            var inputData = new NativeWin32.INPUT[2];
            inputData[0].type = 1;
            inputData[0].ki.wScan = (short)key;
            inputData[0].ki.dwFlags = NativeWin32.KEYEVENTF_SCANCODE;

            inputData[1].type = 1;
            inputData[1].ki.wScan = (short)key;
            inputData[1].ki.dwFlags = NativeWin32.KEYEVENTF_KEYUP | NativeWin32.KEYEVENTF_SCANCODE;

            if (NativeWin32.SendInput(2, inputData, Marshal.SizeOf(typeof(NativeWin32.INPUT))) == 0)
            {
                System.Diagnostics.Debug.WriteLine("SendInput failed with code: " +
                Marshal.GetLastWin32Error().ToString());
            }
